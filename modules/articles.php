 ...
 */
    ...
    public static function save($cols, $files = array())
    {
        ...
        if (isset($cols['authors']) && is_array($cols['authors'])) {
            // ADD: Validate authors completeness
            ...
            if (!$validation['valid']) {
                ...
                trigger('log', array(
                    'action' => 'error',
                    'objectType' => 'article',
                    'content' => 'Author validation failed: ' . implode(', ', $validation['errors'])
                ));
                return false;
            }

            // REPLACE existing line: $newAuthors = $cols['authors'];
            $newAuthors = $validation['users'];
            $cols['authors'] = implode(';', $validation['users']);
        }
        if (isset($cols['id'])) {
            $id = $cols['id'];
            unset($cols['id']);
        ...
        if (!empty($cols['peers']) && isset($cols['versionId'])) {
            ...
            if (!$validation['valid']) {
                ...
                trigger('log', array(
                    'action' => 'error',
                    'objectType' => 'article',
                    'content' => 'Peer validation failed: ' . implode(', ', $validation['errors'])
                ));
                return false;
            }
    ...
    private static function validateUserCompleteness($userIds, $userData = array())
    {
        global $PPHP;
        $db = $PPHP['db'];
        $validUsers = array();
        ...
        foreach ($userIds as $userId) {
            // Check if this is new user data
            if (isset($userData[$userId])) {
                $user = $userData[$userId];

                if (empty($user['email']) || empty($user['name'])) {
                    $errors[] = "Incomplete user data for: " . ($user['email'] ?? 'unknown email');
                    continue;
                }

                if (!filter_var($user['email'], FILTER_VALIDATE_EMAIL)) {
                    $errors[] = "Invalid email format: " . $user['email'];
                    continue;
                }
...
/**
 * Validate user completeness before saving
 *
 * @param array $userIds Array of user IDs
 * @param array $userData Array of new user data
 *
 * @return array Validation result with valid users and errors
 */
